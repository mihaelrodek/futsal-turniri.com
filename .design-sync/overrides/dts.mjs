// forked from design-sync lib/dts.mjs — qualify bare React type names (CSSProperties) against the emitted `import * as React` prelude
//
// The Pitch components spread `...rest` into Chakra primitives, so the checker
// resolves `style?: CSSProperties` from @types/react. The emitted prelude is
// `import * as React from 'react'` and nothing imports `CSSProperties` by
// name, leaving 10 of 25 contracts referencing an undefined type. Only the
// return of emitBody() differs from upstream; see qualifyReactTypes below.
...
function emitBody(type, at, generics, pkgDir) {
  ...
  return { body: qualifyReactTypes(lines.join('\n')), generics, extendsClause: '', prelude: '' };
}

// Type names @types/react exports that the checker prints unqualified. The
// emitted prelude only binds the `React` namespace, so a bare occurrence is an
// undefined type in the contract the design agent reads. Skips names already
// qualified (`React.CSSProperties`) and names inside string-literal unions
// (all lowercase HTML tags — no overlap with this list).
const REACT_TYPES = ['CSSProperties', 'ReactNode', 'ReactElement', 'ElementType', 'ComponentType'];
function qualifyReactTypes(body) {
  let out = body;
  for (const t of REACT_TYPES) out = out.replace(new RegExp(`(?<!\\.|\\w)${t}\\b`, 'g'), `React.${t}`);
  return out;
}
